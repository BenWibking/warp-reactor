...
// SPDX-License-Identifier: BSD-3-Clause
// ABOUTME: Standalone primordial chemistry network ported from AMReX Microphysics
#ifndef INTEGRATORS_PRIMORDIAL_CHEM_HPP
#define INTEGRATORS_PRIMORDIAL_CHEM_HPP
...
constexpr Real k_B = 1.3806490000000002e-16;
constexpr Real m_p = 1.67262192595e-24;
}

inline Real& redshift_storage() {
    static Real value = default_redshift;
    return value;
}
...
: (
   std::exp((-0.16869999999999999)*std::log(std::abs(T)))
    ...
    Real x35 = integrators::math::powi<-3>(x24);
...
: (
   ...
   : (
    ...
    Real x51 = integrators::math::powi<-5>(x24);

    Real x52 = std::exp((-8)*std::log(std::abs(x24)));

    Real x53 = integrators::math::powi<-7>(x24);
...
: (
   0
    ...
    x42 = integrators::math::powi<-3>(x33);
...
: (
   0
    ...
    x61 = integrators::math::powi<-3>(x58);
...
: (
   x186
    ...
    x193 = integrators::math::powi<-5>(x58);
...
: (
   0
    ...
    x31 = integrators::math::powi<-3>(x11);
...
: (
   0
    ...
    x10 = integrators::math::powi<-3>(x8);
...
: (
   0
    ...
    x63 = integrators::math::powi<-3>(x23);
...
: (
   0
    ...
    x131 = integrators::math::powi<-5>(x23);
...
: (
   0
    ...
    x16 = integrators::math::powi<-5>(x8);

    x17 = std::exp((-4)*std::log(std::abs(x8)));

    x18 = ((((x10)*(x10)))*(((x10)*(x10))));

    x19 = integrators::math::powi<-3>(x8);
...
: (
   std::exp((-0.16869999999999999)*std::log(std::abs(T)))
    ...
    x134 = integrators::math::powi<-3>(x100);
...
: (
   ...
   : (
    ...
    x180 = integrators::math::powi<-5>(x100);
    ...
    x189 = integrators::math::powi<-7>(x100);
...
: (
   1
    ...
    x375 = integrators::math::powi<-3>(x0);
...
#endif // INTEGRATORS_PRIMORDIAL_CHEM_HPP
// SPDX-License-Identifier: BSD-3-Clause
